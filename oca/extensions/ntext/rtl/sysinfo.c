...
IsVersionInstalled(LSA_UNICODE_STRING *Handle)
{
    ...
    if (NtOpenKey((PHANDLE)&Handle, 0x2000000u, &ObjectAttributes) >= 0)
    {
        ...
        if (ZwQueryValueKey(
                Handle, &DestinationString, KeyValuePartialInformation, &KeyValueInformation, 0x14u, &ResultLength) >=
                0 &&
            value)
            verification = 1;
        ZwClose(Handle);
...
NTSTATUS
RtlGetOSProductNameStringHelper(PUNICODE_STRING Destination, BOOLEAN verification, PCWSTR Source)
{
...
NTSTATUS
NTAPI
RtlGetOSProductName(PUNICODE_STRING Destination, int verification)
{
    NTSTATUS result;                           // eax@2
...
NTSTATUS
WINAPI
NtQueryLicenseValue(const UNICODE_STRING *name, ULONG *result_type, PVOID data, ULONG length, ULONG *result_len)
{
    static const WCHAR LicenseInformationW[] = {
        'M', 'a', 'c', 'h', 'i', 'n', 'e', '\\', 'S',  'o', 'f', 't', 'w', 'a', 'r', 'e', '\\', 'M',
        'i', 'c', 'r', 'o', 's', 'o', 'f', 't',  '\\', 'W', 'i', 'n', 'd', 'o', 'w', 's', '\\', 'L',
        'i', 'c', 'e', 'n', 's', 'e', 'I', 'n',  'f',  'o', 'r', 'm', 'a', 't', 'i', 'o', 'n',  0};
    ...
    {
        status = NtQueryValueKey(hkey, name, KeyValuePartialInformation, info, info_length, &count);
    ...
    DWORD *max_len)
{
    if (pdata)
    {
        ...
        *max_len *= 2;
        new_data = RtlReAllocateHeap(
            NtCurrentTeb()->ProcessEnvironmentBlock->ProcessHeap, 0, *pdata, *max_len * sizeof(*new_data));
        if (!new_data)
            return FALSE;
    ...
    else
    {
        ...
        *max_len *= 2;
        new_dataex = RtlReAllocateHeap(
            NtCurrentTeb()->ProcessEnvironmentBlock->ProcessHeap, HEAP_ZERO_MEMORY, *pdataex,
            *max_len * sizeof(*new_dataex));
        if (!new_dataex)
            return FALSE;
...
static inline BOOL
LogicalProcessorInfoAddGroupInfo(
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX **pdataex,
    DWORD *len,
    DWORD *pmax_len,
    DWORD num_cpus,
...
// Code based in example on documentation
// https://learn.microsoft.com/pt-br/windows/win32/api/sysinfoapi/nf-sysinfoapi-getlogicalprocessorinformation
SYSTEM_LOGICAL_INFORMATION_FILLED
GetLogicalInfo()
{
    BOOL done = FALSE;
    ...
    while (!done)
    {
        Status = NtQuerySystemInformation(SystemLogicalProcessorInformation, buffer, returnLength, &returnLength);
        ...
        if (!NT_SUCCESS(Status))
        {
            if (Status == STATUS_BUFFER_TOO_SMALL)
            {
                ...
                buffer = (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION)RtlAllocateHeap(
                    NtCurrentTeb()->ProcessEnvironmentBlock->ProcessHeap, HEAP_ZERO_MEMORY, returnLength);
    ...
    DWORD *max_len)
{
    ...
    DbgPrint(
        "Ntext::CreateLogicalProcessorInfo :: %u logical CPUs from %u physical cores across %u packages\n", lcpu_no,
        cores_no, pkgs_no);
    ...
    ULONG *ResultLength)
{
    ...
    DbgPrint(
        "(0x%08x,%p,%u,%p,%u,%p) stub\n", SystemInformationClass, Query, QueryLength, SystemInformation, Length,
        ResultLength);
    ...
    _Out_opt_ DWORD *pulDeviceForm)
{
    ...
    if (pullUAPInfo)
    {
        *pullUAPInfo = ((ULONGLONG)Peb->OSMajorVersion << 48) | ((ULONGLONG)Peb->OSMinorVersion << 32) |
                       ((ULONGLONG)Peb->OSBuildNumber << 16) | (0x1770);
    }
